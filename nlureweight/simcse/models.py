...
def cl_forward(cls,
    ...
    # Get raw embeddings
    if sop_input_ids is not None:
        sop_outputs = encoder(
            sop_input_ids,
            attention_mask=sop_attention_mask,
    ...
    if mlm_outputs is not None and mlm_labels is not None:
        ...
        if mlm_weights is not None:
            if sop_input_ids is not None:
                #print(sop_labels.view(-1))
                sop_loss = loss_fct(sop_pred,sop_labels.view(-1))
                loss = mlm_weights[0] * loss + mlm_weights[1] * masked_lm_loss + mlm_weights[2] * sop_loss
            else:
                loss = (1- mlm_weights) * loss + mlm_weights * masked_lm_loss
        else:
...
class RobertaForCL(RobertaPreTrainedModel):
    ...
    def __init__(self, config, *model_args, **model_kargs):
        ...
        if self.model_args.do_sop:
            self.sop_head = RobertaClassificationHead(config)
